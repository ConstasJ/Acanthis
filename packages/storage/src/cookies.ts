...
export class DatabaseCookieStore implements CookieStore {
    ...
    async setCookies(domain: string, path: string, cookies: Record<string, string>): Promise<void> {
        await this.db.setCookies(domain, path, Object.entries(cookies).map(([name, value]) => ({
            domain,
            path,
            name,
            value,
    ...
    async appendCookie(domain: string, path: string, cookies: Record<string, string>): Promise<void> {
