...
export const ConfigSchema = z.object({
    ...
    impersonate: z.object({
        ...
        profile: z.enum(['chrome149-windows', 'chrome149-linux', 'chrome149-android']).default('chrome149-linux'),
    }),
...
export function getConfig(): Config {
    ...
    try {
        ...
        const envConfig: Record<string, unknown> = {
            ...
            impersonate: {
                enabled: process.env.IMPERSONATE_ENABLED ? process.env.IMPERSONATE_ENABLED === 'true' : undefined,
                profile: process.env.IMPERSONATE_PROFILE as Config['impersonate']['profile'] | undefined,
            },
            flaresolverr: {
                enabled: process.env.FLARESOLVERR_ENABLED ? process.env.FLARESOLVERR_ENABLED === 'true' : undefined,
                host: process.env.FLARESOLVERR_HOST,
                timeoutMs: process.env.FLARESOLVERR_TIMEOUT_MS ? parseInt(process.env.FLARESOLVERR_TIMEOUT_MS, 10) : undefined,
            },
        };
